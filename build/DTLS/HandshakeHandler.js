...
class ClientHandshakeHandler {
							...
							switch (connState.cipherSuite.keyExchange) {
								...
								default:
									this.finishedCallback(
										new Alert_1.Alert(Alert_1.AlertLevel.fatal, Alert_1.AlertDescription.handshake_failure),
	...
	renegotiate() {
		...
		this.sendFlight([hello], [Handshake.HandshakeType.server_hello_done, Handshake.HandshakeType.hello_verify_request]);
	}
			...
			if (this.expectedResponses != null) {
				...
				if (this.expectedResponses.indexOf(lastMsg.msg_type) > -1) {
					...
					if (lastMsg.msg_type === Handshake.HandshakeType.finished) {
						...
						this.bufferHandshakeData(
							...messages.filter((m) => this.needsToHashMessage(m)).map((m) => m.toFragment()) // TODO: avoid unneccessary assembly and fragmentation of messages
						);
					}
					// handle the message
					try {
	...
	computeVerifyData(handshakeMessages, source) {
		const connState = source === 'client' ? this.recordLayer.nextWriteEpoch.connectionState : this.recordLayer.currentReadEpoch.connectionState;
